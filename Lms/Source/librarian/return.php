<?php 
    ...
    if($a > $brdate)
	{
        mysqli_query($link, "insert into finezone values('$id','$username','$utype','$email','$booksname','$fine')");
    }
    $res4 = mysqli_query($link, "select * from issued_book where id=$id");
	...
	$res = mysqli_query($link, "update issued_book set booksreturndate='$a' where id=$id");
	$res2 = mysqli_query($link, "update issue_book set booksreturndate='$a' where id=$id");

	$books_name="";
	$res = mysqli_query($link, "select  * from issued_book where id=$id");
